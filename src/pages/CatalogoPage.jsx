...
function CatalogoPage() {
  ...
  switch (ordenActual) {
    ...
    case "name-desc":
      ...
      break;
    case "price-asc":
      productosOrdenados.sort((a, b) => a.precio_base - b.precio_base);
      break;
    case "price-desc":
      productosOrdenados.sort((a, b) => b.precio_base - a.precio_base);
      break;
    default:
      if (!categoriaActual) {
        productosOrdenados.sort((a, b) => {
          const getRank = (cat) =>
            rankingCategorias.indexOf(cat) === -1
              ? Infinity
              : rankingCategorias.indexOf(cat);
          return getRank(a.categoria) - getRank(b.categoria);
        });
      }
      break;
  }
  ...
  const currentProductsToDisplay = productosOrdenados.slice(
    (paginaActual - 1) * PRODUCTS_PER_PAGE,
    paginaActual * PRODUCTS_PER_PAGE,
  );
  const pageTitle = categoriaActual
    ? categoryNames[categoriaActual] || "Categoría"
    : "Todos los Productos";
  ...
  const paginate = (pageNumber) => {
    if (
      ...
      navigate(`${location.pathname}?${params.toString()}`);
      document
        .querySelector(".catalogo-main-content")
              ...
              {productosOrdenados.length > 0 && (
                <>
                  <p className="product-count">
                    Mostrando {currentProductsToDisplay.length} de{" "}
